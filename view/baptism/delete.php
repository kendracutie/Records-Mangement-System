<?php
// Start session for potential session use
session_start();
...
if (isset($_GET['mid'])) {
    ...
    $fetchQry->bind_param("i", $baptism_id); // Corrected parameter to bind $baptism_id
    $fetchQry->execute();
    ...
    $sql = "UPDATE baptism SET status = 'deleted' WHERE baptism_id = ?";

    // Prepare the statement
    if ($stmt = $cn->prepare($sql)) {
        // Bind the baptism_id as an integer parameter
        $stmt->bind_param('i', $baptism_id);

        // Execute the query
        if ($stmt->execute()) {
            // Log the activity using the session's username
            logActivity($_SESSION['username'], 'Deleted baptism record: ' . $first_name . ' ' . $middle_name . ' ' . $last_name);
...
if (isset($status)) {
    if ($status == 'success') {
        ...
        <script>
          Swal.fire({
            icon: 'success',
            title: '<span style=\"font-family: Arial, Helvetica, sans-serif; font-size: 22px; color:#2F4F4F;\">Baptism record deleted successfully</span>',
            background: '#fffef0', 
        ...
        <script>
          Swal.fire({
            icon: 'error',
            title: '<span style=\"color:#2F4F4F;\">Failed to delete baptism record!</span>',
