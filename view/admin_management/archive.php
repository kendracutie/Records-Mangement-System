...
if (isset($_GET['mid'])) {
    ...
    require_once '../class/Admin.php'; // Include your Admin class
    $Admin = new Admin(); // Instantiate the Admin class

    $archiveAdmin = $Admin->archive($mid); // Call the archive method
...
if (isset($status)) {
    if ($status == 'success') {
        ...
        <script>
          Swal.fire({
            icon: 'success',
            title: '<span style=\"font-family: Arial, Helvetica, sans-serif; font-size: 22px; color:#2F4F4F;\">Admin successfully archived</span>',
            background: '#fffef0', 
        ...
        <script>
          Swal.fire({
            icon: 'error',
            title: '<span style=\"color:#2F4F4F;\">Failed to archive admin!</span>',
